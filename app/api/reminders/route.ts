...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM reminders WHERE event_id = $1', [event_id]
    )
    if ((existing.rowCount ?? 0) > 0) {
      return NextResponse.json({ error: 'Reminder already exists for this event' }, { status: 409 })
    }

    const result = await pool.query(
      `INSERT INTO reminders (event_id, remind_at, channel)
       VALUES ($1, $2, 'email') RETURNING *`,
      [event_id, remindAt.toISOString()]
    )
    ...
    if (process.env.RESEND_API_KEY && process.env.REMINDER_EMAIL) {
      await getResend().emails.send({
        ...
        html: `<p>You'll receive a reminder on <strong>${remindAt.toDateString()}</strong> (7 days before) for:</p>
               <h2>${event.name}</h2>
